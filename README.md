...
├── README.md
├── doc/
│   ├── workflow-api.md        # AI 工作流 API 接口文档
...
```
用户发送消息（新消息自动调用 or 手动点击翻译）
     │
     ▼
┌──────────────┐
│   AI 工作流   │
│  ① 安全检查   │──不安全──► 拦截返回 422（safety_reason）
│  ② 语言检测   │
│  ③ 判断翻译   │
└──────┬───────┘
       │
       ├─ is_translation_requested = true（手动翻译）
       │      │
       │      └─► 只返回翻译结果，不生成推荐回复
       │
       └─ is_translation_requested = false（新消息自动）
              │
              ├─ needs_translation = true → 自动翻译 + 生成推荐回复
              └─ needs_translation = false → 仅生成推荐回复
       │
       ▼
┌──────────────┐    失败     ┌─────────────┐
│ 返回处理结果   │ ─────────► │ fail-open？  │──是──► 放行消息
└──────┬───────┘            │              │
       │                    └──────┬───────┘
       │ 成功                      │ 否
       ▼                           ▼
┌──────────────┐     ┌─────────────┐
│ Redis Pub/Sub │ ──► │ 返回 502    │
│ + List 存储   │     └─────────────┘
└──────────────┘
```

> 完整工作流 API 文档见 [doc/workflow-api.md](doc/workflow-api.md)
